function [SavedParameterSets] = StabilityFeasilityTestII(NumberofTests,NumberofPsets,NumberEquations,EquationParas,ParaLength,SolutionNumer,SolutionDenom,paras,SameSignParas,Jacobian,Species,simpeqnvector,fulleqnvector)
...
for i = 1:NumberEquations
    ...
    SubbedEquation= subs(simpeqnvector,[paras(zero) S],[zeros(size(zero)) 1]);
    CompleteSolution = solve(SubbedEquation, Species);
    SolutionVec = [];
    for i = 1:NumSpp
    eval(['SolutionVec = [SolutionVec CompleteSolution.' char(Species())]);
end
